...
function! DVB_Drag (dir)
    ...
    let l:join_next = ""
    if exists('b:DV_last_drag') && b:DV_last_drag == changenr() - 1
        " Last change was a drag
        let l:join_next = 'undojoin | '
    else
        let b:DV_last_drag = changenr()
    endif

    echomsg b:DV_last_drag
    echomsg l:join_next
    ...
    if mode() ==# 'V'
        ...
        exec l:join_next."nnoremap <silent><expr><buffer>  M  \<SID>Drag_Lines('".a:dir."')"
    ...
    " (to ensure we have access to block position data)...
        ...
        " Set up a temporary convenience...
        exec l:join_next."nnoremap <silent><expr><buffer>  M  \<SID>Drag_Block('".a:dir."')"
